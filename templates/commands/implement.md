...
1. 从仓库根目录运行`{SCRIPT}`并解析FEATURE_DIR和AVAILABLE_DOCS列表。所有路径必须是绝对路径。对于包含单引号的参数，如"I'm Groot"，使用转义语法：例如'I'\''m Groot'（或者可能的话使用双引号："I'm Groot"）。
...
3. 加载并分析实施上下文：
   ...
   - **必需**：阅读plan.md获取技术栈、架构和文件结构
   - **如果存在**：阅读data-model.md获取实体和关系
   - **如果存在**：阅读contracts/获取API规范和测试需求
   - **如果存在**：阅读research.md获取技术决策和约束
   - **如果存在**：阅读quickstart.md获取集成场景

4. **项目设置验证**：
   - **必需**：根据实际项目设置创建/验证忽略文件：

   **检测和创建逻辑**：
   - 检查以下命令是否成功以确定仓库是否为git仓库（如果是，则创建/验证.gitignore）：
...
注意：此命令假设tasks.md中存在完整的任务分解。如果任务不完整或缺失，建议先运行`/speckit.tasks`以重新生成任务列表。
